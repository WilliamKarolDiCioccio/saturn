...
### Macro Parameters: Clear and Consistent

Macro parameters follow the same naming rules as function parameters, with one important twist: they start with an **uppercase letter**. This may seem redundant in simple macros, but it becomes crucial when macros generate larger code expressions. Using uppercase letters for macro parameters makes it immediately clear which identifiers are part of the generated code versus which are literals or fixed parts of the macro.
