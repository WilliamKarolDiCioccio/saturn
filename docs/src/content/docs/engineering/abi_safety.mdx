---
title: ABI Safety
description: Rules and constraints for maintaining Application Binary Interface (ABI) safety in Mosaic’s dynamic library.
---

ABI safety is a **hard requirement** for Mosaic when distributed as a dynamic library. Public engine binaries must remain link-compatible across projects and machines without forcing consumers to rebuild the engine.
...
This isolates layout changes from consumers and is the **preferred pattern** in Mosaic.
...
Mosaic does not attempt to guarantee ABI compatibility across incompatible toolchains. Consumers are expected to use supported configurations.
...
When choosing between convenience and binary safety, Mosaic always favors **explicitness, isolation, and long-term stability**.
