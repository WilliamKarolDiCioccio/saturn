# Package: mosaic/platform

**Location:** `mosaic/include/mosaic/platform/`, `mosaic/src/platform/`
...
- **Entry point macros** — MOSAIC_ENTRY_POINT is standard creation path
...
- ⚠️ **Multiple entry points**: Linker error (only define MOSAIC_ENTRY_POINT once OR android_main)
...
### Files
**Public API:**
- `include/mosaic/core/platform.hpp` — Platform, PlatformContext
- `include/mosaic/core/sys_info.hpp` — SystemInfo, CPUInfo, GPUInfo, MemoryInfo
- `include/mosaic/core/sys_console.hpp` — SystemConsole (create, destroy, print, attach/detachParent)
- `include/mosaic/core/sys_ui.hpp` — SystemUI (messageBox, file dialogs)
- `include/mosaic/platform/Win32/wstring.hpp` — Wide string conversion (Windows)
- `include/mosaic/platform/AGDK/jni_helper.hpp` — JNI utilities (Android)
- `include/mosaic/platform/AGDK/agdk_platform.hpp` — AGDK platform public API
- `include/mosaic/platform/GLFW/glfw_input_mappings.hpp` — GLFW key/button mappings
...
### Build Flags
- `MOSAIC_PLATFORM_WIN32` — Windows platform
- `MOSAIC_PLATFORM_POSIX` — Linux platform
- `MOSAIC_PLATFORM_ANDROID` — Android platform (AGDK)
- `MOSAIC_PLATFORM_EMSCRIPTEN` — Web platform
